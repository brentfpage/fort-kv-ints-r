...
!! 7/15/2025 : the arguments to the mp type constructors were changed
...
!   zeta_mp=mpcmplxdc(cmplx(real(zeta),aimag(zeta)),ndws)
  zeta_mp=mprealq(real(zeta),ndws)+mpcmplx(i,ndws)*mprealq(aimag(zeta),ndws)

  zeta_r_mp=mprealq(real(zeta),ndws)
  zeta_i_mp=mprealq(aimag(zeta),ndws)
...
!  sol(1)=log((vpa2-zeta_mp)/(vpa1-zeta_mp))
  sol(1)=log(sqrt( ( ((vpa2-zeta_r_mp)*(vpa1-zeta_r_mp)+zeta_i_mp**2)/&
       ...
       &             ( (vpa1-zeta_r_mp)**2 +zeta_i_mp**2) )*mpcmplx(i,ndws)

  sol(2)=(vpa2-vpa1)+zeta_mp*sol(1)
  sol(3)=(vpa2**2-vpa1**2)/mpreal(2.0,ndws) + zeta_mp*sol(2)
  sol(4)=(vpa2**3 -vpa1**3)/mpreal(3.0,ndws) + zeta_mp*sol(3)
  sol(5)=(vpa2**4 -vpa1**4)/mpreal(4.0,ndws) + zeta_mp*sol(4)
  sol(6)=(vpa2**5 -vpa1**5)/mpreal(5.0,ndws) + zeta_mp*sol(5)
  ...
  if((vpara(ipara+1,iarb).ge.real(zeta)).and.(vpara(ipara,iarb).lt.real(zeta))) then
     if(aimag(zeta * cos(theta) * k).eq.0.0) then

        if((k * cos(theta)).gt.0.0) then

           sol(1)=sol(1)+mpcmplx(i,ndws)*mppic
           sol(2)=sol(2)+mpcmplx(i,ndws)*mppic*zeta_mp
           sol(3)=sol(3)+mpcmplx(i,ndws)*mppic*zeta_mp*zeta_mp
           sol(4)=sol(4)+mpcmplx(i,ndws)*mppic*&
                & zeta_mp*zeta_mp*zeta_mp
           sol(5)=sol(5)+mpcmplx(i,ndws)*mppic*&
                & zeta_mp*zeta_mp*zeta_mp*zeta_mp
           sol(6)=sol(6)+mpcmplx(i,ndws)*mppic*&
...
! 
     ...
     else if(aimag(zeta * cos(theta) * k).lt.0.0) then

        if((k * cos(theta)).gt.0.0) then

           sol(1)=sol(1)+mpcmplx(i,ndws)*2.0*mppic
           sol(2)=sol(2)+mpcmplx(i,ndws)*2.0*mppic*zeta_mp
           sol(3)=sol(3)+mpcmplx(i,ndws)*2.0*mppic*zeta_mp*zeta_mp
           sol(4)=sol(4)+mpcmplx(i,ndws)*2.0*mppic*&
                & zeta_mp*zeta_mp*zeta_mp
           sol(5)=sol(5)+mpcmplx(i,ndws)*2.0*mppic*&
                & zeta_mp*zeta_mp*zeta_mp*zeta_mp
           sol(6)=sol(6)+mpcmplx(i,ndws)*2.0*mppic*&
                & zeta_mp*zeta_mp*zeta_mp*zeta_mp*zeta_mp
