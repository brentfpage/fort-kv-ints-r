...
!>>
  ...
  function mp_qtor (qa, iprec)
    ...
    integer mpnw
    real (max (mpqknd, kind (1.))), intent (in):: qa
...
!   These are the polynomial package routines.
  ...
  subroutine mp_rpolysolve (nq, polyq, rootapp, root)
    ...
    type (mp_realm), intent(in):: polyq(0:nq), rootapp
    type (mp_realm), intent(out):: root
    integer mpnw, nd1
  ...
  subroutine mp_cpolysolve (nq, polyq, rootapp, root)
    implicit none
    integer, intent(in):: nq
    type (mp_realm), intent(in):: polyq(0:nq)
    type (mp_complexm), intent(in):: rootapp
    type (mp_complexm), intent(out):: root
  ...
  subroutine mp_rpolygcd (n1, poly1, n2, poly2, ng, polyg)
    ...
    type (mp_realm), intent(in):: poly1(0:n1), poly2(0:n2)
    integer, intent(out):: ng
    type (mp_realm), intent(out):: polyg(0:n1)
    integer i, mpnw, nd1
    mpnw = int (poly1(0)%mpr(1))
    mpnw = min (mpnw, mpwdsm)
    nd1 = mpwdsm
    do i = 0, n1; polyg(i)%mpr(0) = mpwdsm6; enddo
  ...
  subroutine mp_rpolydiv (n1, poly1, n2, poly2, n3, poly3, n4, poly4)
    ...
    type (mp_realm), intent(out):: poly3(0:n1), poly4(0:n1)
    integer i, mpnw, nd1
    mpnw = int (poly1(0)%mpr(1))
    mpnw = min (mpnw, mpwdsm)
    nd1 = mpwdsm
    do i = 0, n1; poly3(i)%mpr(0) = mpwdsm6; enddo
    do i = 0, n1; poly4(i)%mpr(0) = mpwdsm6; enddo
  ...
  subroutine mp_rpolyeval (np, poly, arg, result)
    ...
    type (mp_realm), intent(in):: poly(0:np), arg
    type (mp_realm), intent(out):: result
    integer mpnw, nd1
    result%mpr(0) = mpwdsm6
    mpnw = int (arg%mpr(1))
    mpnw = min (mpnw, mpwdsm)
    nd1 = mpwdsm
  ...
  subroutine mp_cpolyeval (np, poly, arg, result)
    implicit none
    integer, intent(in):: np
    type (mp_realm), intent(in):: poly(0:np)
    type (mp_complexm), intent(in):: arg
    type (mp_complexm), intent(out):: result
    integer lc1, mpnw, nd1
    lc1 = arg%mpc(0)
    mpnw = max (int (arg%mpc(1)), int (arg%mpc(lc1+1)))
    mpnw = min (mpnw, mpwdsm)
    nd1 = mpwdsm
    result%mpc(0) = mpwdsm6
    result%mpc(lc1) = mpwdsm6
